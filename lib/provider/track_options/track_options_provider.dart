...
import 'package:spotube/provider/metadata_plugin/tracks/recommendation_provider.dart';
...
class TrackOptionsActions {
  ...
  Future<void> actionStartRadio(BuildContext context) async {
    ...
    // Try track.radio() first, but fall back to recommendation provider
    // if it returns empty or fails
    List<SpotubeFullTrackObject> tracks;
    try {
      tracks = await metadataPlugin.track.radio(track.id);
    } catch (e, stack) {
      AppLogger.reportError(e, stack);
      tracks = [];
    }

    // If radio returned empty/insufficient results, use recommendation fallback
    if (tracks.length < 3 && track is SpotubeFullTrackObject) {
      try {
        final recommendations = await ref.read(
          trackRecommendationProvider(track as SpotubeFullTrackObject).future,
        );
        // Merge: keep any valid radio results, then add recommendations
        final seenIds = <String>{track.id, ...tracks.map((t) => t.id)};
