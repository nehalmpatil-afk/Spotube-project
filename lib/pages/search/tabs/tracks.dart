...
class SearchPageTracksTab extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    /// Asynchronously fetches recommendations for the given track and adds
    /// them to the queue. Does not block playback — failures are silent.
    void loadRecommendationsAsync(SpotubeFullTrackObject track) {
      ref.read(trackRecommendationProvider(track).future).then(
        (recommendations) {
          if (recommendations.isNotEmpty) {
            playlistNotifier.addTracks(recommendations);
          }
        },
        onError: (e, stack) {
              ...
              if (isRemoteDevice) {
                ...
                if (!isTrackPlaying && context.mounted) {
                  ...
                  if (shouldPlay) {
                    ...
                    // Fetch and add related tracks asynchronously
                    loadRecommendationsAsync(track);
                  }
                }
              }
            },
